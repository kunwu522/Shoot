...
- (BOOL)canSelectItemAtIndexPath:(NSIndexPath *)indexPath {
  ...
  BOOL hasContent = [self.calendar component:NSCalendarUnitDay fromDate:[self getEffectiveDateFromIndexPath:indexPath]] % 5 == 3;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  if ([self.calendar component:NSCalendarUnitDay fromDate:date] % 5 == 3) {
...
- (NSDate *) getImageDisplayDate {
    ...
    if (components.weekday == 7) {
        return [self.selectedDate dateByAddingTimeInterval:60 * 60 * 24];
    } else {
        return [self.selectedDate dateByAddingTimeInterval:60 * 60 * 24 * (7 - components.weekday % 7 + 1)];
    }
}
